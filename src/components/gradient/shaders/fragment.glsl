...
void main() {
    ...
    float t2 = uTime * .5;
    color = texture(uBG,vec2(
        pnoise(vec3(uv.rg * 10.0, dot(uv,.001 * vec2(sin(length((uv * .01 + t2)/min(t2 ,1.0))))))),
        pnoise(vec3(uv.gr * 10.0, dot(uv.gr,.001 * vec2(cos(length((uv * .01 + t2)/min(t2 ,1.0)))))))
    )).rgb;
