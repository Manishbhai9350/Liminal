...
void main() {
    ...
    float noised = pnoise(vec3((vUv - uMouse * .05) * uFreq * .1,uTime * .1 * uSpeed));
    float n2 = pnoise(vec3((vec2(100.0) + vUv) * uFreq * .1,uTime * .1 * uSpeed));
    float n3 = pnoise(vec3((vec2(noised,n2) * uFreq + vUv) * uFreq * .1,uTime * .1 * uSpeed));
