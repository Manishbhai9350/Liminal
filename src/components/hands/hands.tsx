...
export function Arm1(props: HandProps) {
  const { nodes } = useGLTF("/models/fixedarm.glb") as unknown as ArmResult;
  ...
  const Uniforms = useMemo(
    () => ({
      ...
      uCamera: new THREE.Uniform(new THREE.Vector3().copy(camera.position)),
      uBottom: new THREE.Uniform(0),
  ...
  const MatRef = useRef<CSM<typeof THREE.MeshStandardMaterial>>(null);
  ...
  useFrame((_, delta) => {
    ...
    groupRef.current.rotation.y = baseRotation.y + rotationRef.current.x * 0.5;
  ...
  useEffect(() => {
    ...
    MatRef.current.uniforms.uCamera.value.copy(camera.position);
  ...
  useEffect(() => {
    ...
    nodes.Point_Std_Skin_Arm_0.updateMatrixWorld();
    const box = new THREE.Box3().setFromObject(nodes.Point_Std_Skin_Arm_0);

    MatRef.current.uniforms.uCamera.value.copy(camera.position);
    MatRef.current.uniforms.uBottom.value = box.min.y;
    MatRef.current.uniforms.uTop.value = box.max.y;
  }, [nodes.Point_Std_Skin_Arm_0, camera.position]);

  useEffect(() => {
    if (!MatRef.current || !groupRef.current) return;

    gsap.set(groupRef.current.position, {
      y: -sceneBounds.height - 5,
      delay: 1,
      duration: 3,
      ease: "power3.out",
    });
    ...
    gsap.to(groupRef.current.position, {
      ...
      onComplete() {
        ...
        if (MatRef.current) {
          MatRef.current.uniforms.uBottom.value = box.min.y;
          MatRef.current.uniforms.uTop.value = box.max.y;
          MatRef.current.uniforms.uAnimated.value = 1.0;
...
export function Hand1(props: HandProps) {
  ...
  const scroll = useScroll();
  const base = "kulack";
  const sequence = useMemo(() => ["One", "Rock_finger", "Three"], []);
